...
def process_uploaded_file(uploaded_file):  
    ...
    Args:  
        uploaded_file (BytesIO): ストリームリットのファイルアップローダーからの音声ファイル  
...
with tab2:  
    if st.button("音声を自動処理"):  
        with st.spinner('処理中...'):  
            ...
            st.session_state.audio_data = processed_audio  # session_stateのaudio_dataを更新  
            if volume_normalize:  
...
with tab4:  
    ...
    if st.session_state.processed_audio:  
        ...
        metadata = st.text_area("メタデータ (タイトル、説明など)",   
